...
// Create a singleton Redis instance with enhanced error handling
class RedisClient {
  constructor() {
    this.client = null;
  ...
  initializeClient() {
    try {
      ...
      // Upstash Redis is HTTP-based, so it doesn't maintain persistent connections
  ...
  async executeCommand(commandName, operation) {
    if (!this.isConnected || !this.client) {
      console.warn(`[${new Date().toISOString()}] Redis client not connected. Skipping ${commandName} operation.`);
      return null;
    }
