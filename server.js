...
app.get("/generate-link", async (req, res) => {
    try {
        ...
        if (!rows || rows.length === 0) return res.status(404).json({ error: "No data found in Google Sheet" });
...
app.post("/create-crypto-invoice", async (req, res) => {
    try {
        ...
        if (!priceAmount || !botName || !itemNumber) return res.status(400).json({ error: "Missing required fields" });
        ...
        if (!data.invoice_url) return res.status(400).json({ error: "Failed to create invoice", details: data });
